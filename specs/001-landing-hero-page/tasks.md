...
- [X] T001 Confirm skill/UI icon packs are installed: `@iconify-json/material-symbols` and `@iconify-json/simple-icons` are both present in package.json (both installed as of 2026-07-09) — run `npm install` if a fresh checkout lacks them
...
- [X] T007 Implement src/layouts/Layout.astro: `<html lang>`, head meta + `<title>`/description props, Inter + JetBrains Mono `<link>`s, inline no-flash theme init script (localStorage → prefers-color-scheme → light, set `data-theme` before paint), `no-transition`→`is-ready` toggle on first `requestAnimationFrame`, global.css import, `<slot/>` (FR-008)
- [X] T008 Create home page shell src/pages/index.astro: use Layout, add "Skip to content" link, a `<main id="home">` container that is full-width with gutters on mobile and centered at `--container-wide` (1200px, home-sections container per .skills/design.md §4.1) from `lg` up (FR-015); leave slots to compose Hero/Experience/Skills/NavBar in later phases

**Checkpoint**: Tokens, theme, content schema, and responsive page shell exist — user stories can begin.

---

## Phase 3: User Story 1 - Understand who the person is and reach key actions (Priority: P1) 🎯 MVP

**Goal**: A visitor lands on `/` and immediately sees the tagline, greeting, identity (Full
Stack Developer), a short description, and two CTAs (View Projects → `/projects`, View CV →
`/cv`).
...
- [X] T010 [US1] Implement src/components/Hero.astro (props: profile entry) rendering, in order, tagline → greeting → `<h1 id="hero-title">` name → role → description → two CTAs → placeholder portrait `<img>` with explicit width/height + descriptive alt (FR-020); CTA icons via `<Icon name="material-symbols:...">` marked `aria-hidden`; each CTA a ≥44px target with distinct hover/focus/active states (FR-003); apply `data-reveal` with staggered `--reveal-delay` (FR-001/002/003/012); style CTAs as primary/secondary per .skills/design.md §7.2
- [X] T011 [US1] Load the `profile` entry in src/pages/index.astro (`getEntry`) and render `<Hero>` as the first child of `<main>` (FR-001)

**Checkpoint**: Home page delivers the MVP — identity + both CTAs — independently testable.
...
- [X] T013 [US2] Implement src/components/NavBar.astro: fixed, centered bottom `<nav aria-label="Primary">` with four icon links (Home `/`, About `/about`, Projects `/projects`, Contact me `/contact`) using `material-symbols` icons + `aria-label` + tooltip, `aria-current="page"` on Home, ≥44px targets, separator, and embedded `<ThemeToggle/>`; active route shown as a highlighted pill (glow/highlight) + `aria-current="page"` — distinguished by shape + fill + `aria-current`, not color alone — NOT a magnify/scale effect (motion minimal, reduced-motion-safe); pill shape `--radius-full` (.skills/design.md §7.4) on a solid `--surface` fill + hairline border + shadow (§7.1 "solid fill preferred"); at very narrow widths hide lower-priority links in order (Projects, then Contact me) via CSS media queries so the bar never overflows while Home/About/toggle stay reachable — no JS overflow menu (FR-004/017/018/014)
...
`{short_month}-{year}` start–end) and a set of icon+text skill pills.
...
- [X] T015 [P] [US3] Author three experience entries src/content/experience/company-a.md, company-b.md, company-c.md (frontmatter: company, position, photo, start `MMM-yyyy`, end `MMM-yyyy`|`Present`, order 1–3) validating against the `experience` schema (FR-009/010)
...
- [X] T017 [P] [US3] Implement src/components/ExperienceCard.astro (props: company, position, photo, photoAlt?, start, end) showing only those fields, rendering `start — end`, with `<img>` alt (default `${company} logo`) + explicit dimensions so text stays readable if the image fails (FR-009, Edge Cases)
...
- [X] T019 [P] [US3] Implement src/components/SkillPill.astro (props: name, icon?) rendering a pill = optional `<Icon name={icon} aria-hidden>` + `{name}` text; when no suitable brand icon is provided it MUST render text-only without breaking layout (FR-011); pill shape per .skills/design.md §7.4, legible in both themes
...
- [X] T022 [P] Accessibility pass: verify single `<h1>`, landmark regions, skip link, keyboard reachability + visible focus for all nav links/toggle/CTAs, `aria` on icon-only controls, `aria-current="page"` on the active route, and descriptive alt text on the portrait + experience photos (FR-014/017/019/020, quickstart §13)
- [X] T023 [P] Contrast + theme pass: confirm AA text/UI contrast in BOTH light and dark themes, including skill brand marks, and no theme flash on first load (FR-008, SC-007, quickstart §8/§15)
- [X] T024 [P] Responsive pass: verify no horizontal scroll and correct full-width→`--container-wide` (1200px) behavior from 320px through 1536px+, that the nav bar never obscures actions, and that at very narrow widths Projects then Contact me collapse out while Home/About/toggle stay reachable and the bar does not overflow (FR-015/018, SC-006, quickstart §8a/§14)
- [X] T025 [P] Reduced-motion pass: with `prefers-reduced-motion`, confirm all content is immediately visible and no reveal animation runs (FR-013, SC-005, quickstart §12)
- [X] T026 Copy/tone review: verify all authored content (profile, experience, skills) reads calm, confident, and professional, avoids hype/superlatives/sales language, and has no redundancy, consistent with .skills/design.md and FR-016
- [X] T027 Remove leftover starter file src/components/Welcome.astro if present, and confirm no unused imports remain
- [X] T028 Run `npm run build` (must succeed — validates Zod content + icon names + types), then `npm run preview` and execute the quickstart.md validation scenarios end to end
- [X] T029 [P] Performance & SEO pass: run Lighthouse against the built `/` page and confirm the Constitution II / .skills/design.md budgets are met — Performance ≥ 95, SEO 100, LCP < 1.5s, CLS < 0.05 (explicit media dimensions on portrait + logos), and effectively zero client JS beyond the theme island (Constitution II, Quality Gate 5)
...
- **User Stories (Phase 3–5)**: All depend on Foundational. US1 (P1) is the MVP; US2 and US3
...
- **US3 leaf work**: T015, T016, T017, T019 are all `[P]` (distinct files) — build them
  together, then T018 → T020 → T021.
- **Polish**: T022–T025 and T029 are all `[P]` (independent verification passes).
...
3. US2 → validate nav bar + theme → demo.
4. US3 → validate experience + skills → demo.
5. Polish (T022–T029) → final build + quickstart validation.

### Parallel Team Strategy

After Foundational: Dev A → US1, Dev B → US2, Dev C → US3. Coordinate only on the shared
...
- `/about`, `/projects`, `/contact`, `/cv` routes are out of scope — tasks only ensure the
