...
Rationale: MINOR — added a mandatory CSS authoring standard requiring all Tailwind
work to follow `.skills/tailwind-readability.md`, expressed as expanded guidance in
the Styling constraint and a new Quality Gate 8 (CSS readability). Also registered
`.skills/tailwind-readability.md` as a governed dependent artifact.
(1.0.1 was a PATCH that relocated `.skills/design.md` and reconciled references.
 1.0.0 was the initial ratification of the project constitution — MAJOR baseline.)
...
Templates requiring updates:
  ...
  ✅ .specify/templates/tasks-template.md — reviewed; no constitution/testing
     coupling, no changes required.
  ✅ .skills/design.md — aligned; constitution references it as the design source of truth.
  ✅ .skills/tailwind-readability.md — registered as the mandatory CSS authoring standard
     (Styling constraint + Quality Gate 8).

Follow-up TODOs: None. Ratification date set to initial adoption date.
2026-07-23 (1.0.1): `design.md` moved to `.skills/design.md`; all references updated.
2026-07-23 (1.1.0): Adopted `.skills/tailwind-readability.md` as the project CSS standard.
-->
...
`.skills/design.md`: prose capped to its measure (`~65ch` / `--container-prose`), a single-column
...
from `.skills/design.md` (Performance ≥ 95, Accessibility 100, SEO 100, LCP < 1.5s, CLS < 0.05)
...
single layout tier below the `lg` (1024px) breakpoint per `.skills/design.md`; distinct
...
subtle fades and lifts described in `.skills/design.md`, implemented with Motion One, and MUST
...
- **Language**: TypeScript as the primary language for components, config, and content
  schemas.
- **Styling**: Tailwind CSS, mapped to the design tokens defined in `.skills/design.md`.
  All Tailwind authoring MUST follow the readability and maintainability standard in
  `.skills/tailwind-readability.md` (logical utility ordering, readable multi-line grouping
  for long class lists, extraction of thrice-repeated utility strings, clean conditional
  classes, and keeping genuinely custom values in scoped CSS rather than bracket
  arbitrary-value utilities). Where a token has no exact Tailwind equivalent, the value MUST
  live in scoped CSS as described in that guideline.
...
and contact. `.skills/design.md` is the authoritative source for visual and interaction design;
where this constitution and `.skills/design.md` disagree, this constitution governs and `.skills/design.md`
MUST be reconciled.
...
5. **Performance**: Lighthouse budgets from `.skills/design.md` met on reading pages.
...
7. **Print/CV export**: `/resume` and `/cv` produce ATS-friendly, selectable-text output
   per `.skills/design.md` §10.1.
8. **CSS readability**: Tailwind usage conforms to `.skills/tailwind-readability.md` —
   utilities are consistently ordered, long class lists are grouped for readability,
   repeated utility strings are extracted, and no bracket arbitrary-value utilities stand in
   for a missing token (such values live in scoped CSS).
...
conflicts, this document governs and dependent artifacts (`.skills/design.md`,
`.skills/tailwind-readability.md`, spec-kit templates) MUST be reconciled to it.
